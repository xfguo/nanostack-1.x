 ...
 *
 * \file main.c
 * \brief Example bare skeleton for starting a new Micro series application.
 *
 */
...
int main( void )
{
	/* Initialize the Micro hardware */
	LED_INIT();
	bus_init();

	/* Setup the application task and start the scheduler */
	xTaskCreate( vAppTask, "App", configMINIMAL_STACK_SIZE+200, NULL, (tskIDLE_PRIORITY + 2 ), NULL );
	vTaskStartScheduler();
