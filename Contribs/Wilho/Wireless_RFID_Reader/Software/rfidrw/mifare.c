...
/* Configure PN512 and select one individual tag. */
void MifareSelect(uint8_t *dta)
{
...
void ReadMifare(uint8_t *dta)
{
	...
	/* Execute identification and selection procedure. */
	MifareSelect(dta);
	...
	if (type == 0x04)
	{
		...
		*(dta+7) = GR(FIFODATAREG);	//Read SNR 7
		...
		{    	
			...
			/* If pages 8-11 are done, go to block 12. */ 	        		
			if (page == 12 && cnt == 8)
				cnt = 40;
...
/* Write Mifare Classic or UltraLight tag. */
void WriteMifare(uint8_t rxdata[])
{
	uint8_t i = 0;
	uint8_t k = 0;
	uint8_t ack = 0;
	...
	/* Come back here to write data again in case of a NACK. */			    
	rewrite:
	...
	/* If the SAK response was 08h, write a Classic tag. */
	if (type == 0x08)
	{
		success = 1;
		//debug("1k ");
		...
		/* Active state: perform write command to Classic tag. */
		SR(FIFOLEVELREG, 0x80);	//Flush FIFO
		SR(TXMODEREG, 0x80);	//Activate CRC
		SR(RXMODEREG, 0x80);	//Activate CRC   
		SR(FIFODATAREG, 0xA0);	//Write command
		SR(FIFODATAREG, block);	//Block number to write
		...
		GR(ERRORREG);			//Read ErrReg
		GR(FIFOLEVELREG);		//Read FIFOLevel
		GR(FIFODATAREG);		//Read AK/NAK
		ack = GR(0x00);			//Save response

		/* If NAK received, go back and try again. */
		if (ack != 0x0A)
		{
			goto rewrite;
		}
	
		/* AK received, continue writing. */
		else
		{	
			LED_OFF();
			ack = 0x00;
		...
		/* Write one block (16 bytes) of NDEF data. */
		for (i = 0; i <= 15; i++)
		{    
			/* Get data byte from array and write it to FIFO buffer. */
			SR(FIFODATAREG, rxdata[k]);
        
			/* Check if the byte is NDEF end mark. */
			if (rxdata[k] == 0xFE)
				success = 3;
		...
		if (ack != 0x0A)
		{
			k = k - 16;	//Update to write previous block again
			goto rewrite;
		}
		...
		/* Update to next block number, if writing all data is not finished. */
		if ((block >= 1) && (block <= 4) && (success != 3))
		{
			vTaskDelay(5 / portTICK_RATE_MS );
		
			/* Skip the sector block. */
			if (block == 2)
				block++;
	...
	/* If the SAK response was 04h, write an UltraLight tag. */
	if (type == 0x04)
	{
		success = 1;
		...
		/* Active state: perform write command to UltraLight tag. */
		SR(FIFOLEVELREG, 0x80);	//Flush FIFO
		SR(FIFODATAREG, 0xA2);	//Write command
		SR(FIFODATAREG, page);	//Page number to write
		SR(TXMODEREG, 0x80);	//Activate CRC
		SR(RXMODEREG, 0x80);	//Activate CRC
		vTaskDelay(5 / portTICK_RATE_MS );
  	
		/* Write one page of NDEF data. */
		for (i = 0; i <= 3; i++)
		{    
			/* Set the correct tag memory size to NDEF header. */
			if (k == 2 && page == 4)
				SR(FIFODATAREG, 0x08);
		...
		GR(FIFODATAREG);		//Read AK/NAK
		ack = GR(0x00);			//Save response
		ack &= ~0xF0;			//Only LSB matters
	
		/* If NAK received, go back and try again. */
		if (ack != 0x0A)
		{
			k = k - 4;	//Update to write previous page again
			goto rewrite;
		}
		...
		/* Update to next page number, if writing all data is not finished. */
		if ((page <= 14) && (success != 3))
		{
			page++;
			vTaskDelay(10 / portTICK_RATE_MS );
			goto rewrite;
	...
	/* If there's no tag in the field, go back and try again. */
	if ((success == 0))
	{
		debug("No tag. ");
		goto rewrite;
