...
portCHAR adc_read(uint8_t *channels, uint8_t n_channels)
{
	...
	{	/*allocate buffer to channel and enable ADC*/
		ADC12MCTL[i] = channels[i] | SREF_1;
		P6SEL |= (1 << i);
	}
	ADC12MCTL[n_channels-1] |= 0x80 ; /*mark end of sequence, use internal reference voltage*/
//	ADC12CTL0 = SHT10 | SHT00 | MSC | ADC12ON;	
	ADC12CTL0 = SHT10 | SHT00 | MSC | ADC12ON | REFON;	//use REFON = 1.5 V, REF2_5V = 2.5V	
	...
	ADC12CTL0 |=  ENC | ADC12SC;	//ADC12SC starts 
	/*Start*/
